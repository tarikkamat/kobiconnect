...
export function ChannelPricingManager({
    ...
    >(() => {
        ...
        connections.forEach((conn) => {
            initial[conn.id] =
                DEFAULT_COMMISSIONS[conn.marketplace.toLowerCase()] ?? 15;
        });
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                        Ürünün hangi pazaryerlerinde satışa açılacağını seçin ve
                        komisyon oranlarına göre önerilen satış fiyatını
                        hesaplayın.
                    </p>
                </div>
            </div>
            ...
            {connections.length === 0 ? (
                <div className="space-y-1 rounded-xl border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
                    <Store className="mx-auto mb-1 size-6 opacity-40" />
                    <p className="font-medium text-foreground">
                        Henüz bağlı bir satış kanalı bulunmuyor.
                    </p>
                    <p>
                        Kanallar menüsünden Trendyol, Hepsiburada vb.
                        mağazalarınızı bağlayabilirsiniz.
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    {connections.map((conn) => {
                        ...
                        const suggestedPrice =
                            calculateSuggestedPrice(commission);
                        const commissionAmount =
                            suggestedPrice > numBasePrice
                                ? Math.round(
                                      (suggestedPrice - numBasePrice) * 100,
                                  ) / 100
                                : 0;
                        ...
                        return (
                            <div
                                key={conn.id}
                                className={cn(
                                    'space-y-3 rounded-xl border bg-card p-3.5 transition-all',
                                    isSelected
                            ...
                            >
                                ...
                                {isSelected && (
                                    <div className="animate-in space-y-2.5 border-t border-border/60 pt-2 duration-100 fade-in">
                                        ...
                                        <div className="space-y-1.5 rounded-lg bg-secondary/30 p-2.5">
                                            <div className="flex items-center justify-between text-[11px]">
                                                <span className="text-muted-foreground">
                                                    Taban (Net) Fiyat:
                                                </span>
                                                <span className="font-mono font-medium">
                                                    {numBasePrice.toFixed(2)} TL
                                                </span>
                                            </div>
                                            <div className="flex items-center justify-between text-[11px]">
                                                <span className="text-muted-foreground">
                                            ...
                                            <div className="flex items-center justify-between border-t border-border/50 pt-1 text-xs font-semibold text-foreground">
                                        ...
                                        {onApplySuggestedPrice &&
                                            suggestedPrice > 0 && (
                                                <Button
                                                    ...
                                                    className="h-6 w-full gap-1 text-[11px] text-primary hover:bg-primary/10"
                                                >
                                                    <ArrowRight className="size-3" />
                                                    Önerilen Fiyatı Ürün
                                                    Fiyatına Uygula (
                                                    {suggestedPrice.toFixed(2)}{' '}
                                                    TL)
                                                </Button>
