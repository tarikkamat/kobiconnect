...
export default function UnitEdit({ unit }: { unit: UnitData }) {
    const [name, setName] = useState(unit.name);
    const [shortName, setShortName] = useState(unit.shortName);

    return (
        <>
            <Head title={`Birimi Düzenle: ${unit.name}`} />
            ...
            <div className="mx-auto flex max-w-7xl flex-col gap-5 p-4 font-sans sm:p-6 lg:p-8">
                ...
                {/* Form */}
                <Form
                    {...UnitController.update.form({ unit: unit.id })}
