...
import { ArrowLeft, Layers, Loader2, Pencil, Save } from 'lucide-react';
...
import { index, show } from '@/routes/product-groups';
...
export default function ProductGroupEdit({ group }: { group: GroupData }) {
    ...
    return (
        <>
            <Head title={`Ürün Grubunu Düzenle: ${group.name}`} />
            ...
            <div className="mx-auto flex max-w-7xl flex-col gap-5 p-4 font-sans sm:p-6 lg:p-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    >
                        <Link href={show({ productGroup: group.id })}>
                            <Layers className="size-3.5" />
                            Grup Ürünlerini Yönet
                        </Link>
                    </Button>
                </div>
                ...
                <div className="rounded-xl border border-border bg-card p-4 shadow-xs sm:p-5">
                    <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Ürün grubu adı, slug ve açıklama bilgilerini
                            güncelleyin.
                        </p>
                    </div>
                </div>
                ...
                {/* Form */}
                <Form
                    {...ProductGroupController.update.form({
                ...
                >
                    {({ processing, errors }) => (
                        <>
                            {/* Preserve existing product IDs in hidden inputs */}
                            {group.productIds.map((pId, i) => (
                                <input
                                    key={pId}
                                    type="hidden"
                                    name={`product_ids[${i}]`}
                                ...
                                <Card className="gap-0 overflow-hidden border-border bg-card py-0 shadow-xs">
                                    <CardHeader className="border-b border-border bg-muted/40 px-4 py-3">
                                        ...
                                        <CardDescription className="text-xs">
                                            Grup adı, benzersiz kodu ve açıklama
                                            metnini düzenleyin.
                                        </CardDescription>
                                    </CardHeader>
