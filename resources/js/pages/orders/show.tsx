            ...
            <div className="flex flex-col gap-5 p-4 sm:p-6 lg:p-8 font-sans max-w-7xl mx-auto">
                ...
                <div className="rounded-xl border border-border bg-card p-4 sm:p-5 shadow-xs">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                                ...
                                {order.lastModifiedAt && (
                                    <div className="flex items-center gap-1 text-muted-foreground/80">
                                        ...
                                        <span>Son Güncelleme: {order.lastModifiedAt}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {/* Marketplace Tag */}
                        {order.marketplace && (
                            <div className="flex items-center gap-3 self-start md:self-center rounded-lg border border-border/80 bg-secondary/30 px-3.5 py-2">
                                <MarketplaceLogo
                    ...
                    <div className="flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/10 p-3.5 text-sm text-warning shadow-xs">
                        ...
                        <div className="space-y-0.5">
                            ...
                            <p className="text-xs text-warning/90">
                                Stok ayrıldı, ancak bu siparişi henüz kargoya vermeyiniz. Ödemesi onaylanmadan gönderilen siparişlerin iptal riskini pazaryeri karşılamamaktadır.
                            </p>
                        </div>
                    </div>
                )}
                ...
                {unmatched > 0 && (
                    <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3.5 text-sm text-destructive shadow-xs">
                        ...
                        <div className="space-y-0.5">
                            <strong className="font-semibold text-xs">
                                {unmatched} adet satır katalogdaki ürünle eşleşmedi
                            </strong>
                            <p className="text-xs text-destructive/90">
                                Sipariş eksiksiz kaydedildi; ancak eşleştirme yapılana kadar stok bu satırlar için düşülemez.
                            </p>
                        </div>
                    </div>
                )}
                        ...
                        <div className="overflow-hidden rounded-xl border border-border bg-card shadow-xs">
                            <div className="flex items-center justify-between border-b border-border bg-muted/40 px-4 py-3">
                                ...
                                <span className="text-xs text-muted-foreground font-medium">
                                    Toplam {lines.reduce((acc, l) => acc + l.quantity, 0)} adet ürün
                                </span>
                            </div>
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-b border-border hover:bg-transparent text-xs">
                                        ...
                                        <TableHead className="min-w-[180px]">Ürün Bilgisi</TableHead>
                                        <TableHead className="text-center">Adet</TableHead>
                                        <TableHead className="text-right">Birim Fiyat</TableHead>
                                        <TableHead className="text-right">Komisyon / KDV</TableHead>
                                        <TableHead className="text-right pr-4">Tutar</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {lines.map((line) => (
                                        <TableRow key={line.id} className="hover:bg-secondary/30">
                                                ...
                                                <div className="space-y-0.5">
                                                    ...
                                                    <div className="pt-0.5 flex flex-wrap items-center gap-1.5">
                                                        {line.matched ? (
                                                            <Badge
                                                                variant="outline"
                                                                className="text-[10px] font-normal text-success border-success/30 bg-success/5"
                                                            >
                                            ...
                                            <TableCell className="text-center font-mono text-xs py-3 align-top tabular-nums">
                                                <span className="font-semibold">{line.quantity}</span>
                                            </TableCell>

                                            <TableCell className="text-right font-mono text-xs py-3 align-top tabular-nums">
                                                <span>{line.unitPrice}</span>
                                            </TableCell>

                                            <TableCell className="text-right text-xs py-3 align-top space-y-0.5 tabular-nums">
                                                {line.commission && (
                                                    <div className="text-destructive font-medium text-xs">
                                                        {line.commission}
                                                        {line.commissionAmount && ` (${line.commissionAmount})`}
                                                    </div>
                                                )}
                                                {line.vatRate && (
                                            ...
                                            <TableCell className="text-right font-mono font-bold text-xs py-3 pr-4 align-top tabular-nums text-foreground">
                            ...
                            <CardHeader className="border-b border-border bg-muted/40 px-4 py-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <CardTitle className="text-sm font-semibold">
                                        Kargo & Gönderi Paketleri ({packages.length})
                                    </CardTitle>
                                </div>
                            </CardHeader>
                            <CardContent className="p-4 space-y-3">
                                {packages.length === 0 ? (
                                    <div className="py-4 text-center text-xs text-muted-foreground">
                                        Bu siparişe ait kargo paketi kaydı henüz oluşmadı.
                                    </div>
                                ) : (
                                    packages.map((pkg) => (
                                        <div
                                            key={pkg.id}
                                            className="rounded-lg border border-border bg-card p-3.5 space-y-2.5"
                                        >
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
                                                {pkg.trackingNumber && (
                                                    <div>
                                                        <span className="block text-muted-foreground text-[11px]">Takip Numarası:</span>
                                                        <div className="flex items-center gap-1.5 pt-0.5">
                                                            ...
                                                            <button
                                                                type="button"
                                                                onClick={() => copyTrackingNumber(pkg.trackingNumber!, pkg.id)}
                                                ...
                                                {pkg.deci && (
                                                    <div>
                                                        <span className="block text-muted-foreground text-[11px]">Desi:</span>
                                                ...
                                                {pkg.shippedAt && (
                                                    <div>
                                                        <span className="block text-muted-foreground text-[11px]">Kargoya Veriliş:</span>
                                                ...
                                                {pkg.deliveredAt && (
                                                    <div>
                                                        <span className="block text-muted-foreground text-[11px]">Teslim Tarihi:</span>
                                                        <span className="font-sans text-foreground text-xs tabular-nums">
                                                            {pkg.deliveredAt}
                                                        </span>
                                                    </div>
                        ...
                        <Card className="gap-0 py-0 overflow-hidden border-border bg-card shadow-xs">
                            ...
                            <CardContent className="p-4">
                                {history.length === 0 ? (
                                    ...
                                    <ol className="relative border-l border-border/80 ml-2.5 space-y-3 my-1">
                                        {history.map((entry) => (
                                            <li key={entry.id} className="ml-4 relative">
                                                <span className="absolute -left-[23px] top-1 flex size-3 items-center justify-center rounded-full bg-primary/20 ring-3 ring-card">
                                                ...
                                                <span className="text-[10px] text-muted-foreground">
                                                    Kaynak: {entry.source === 'pull' ? 'Pazaryeri Senkronu' : entry.source}
                                                </span>
                                            </li>
                                        ))}
                                    </ol>
                    ...
                    <div className="space-y-5">
                            ...
                            <CardContent className="p-4 space-y-3">
                                    ...
                                    {financials ? (
                                        <>
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">Brüt Satış Tutarı:</span>
                                            ...
                                            {financials.discount !== '0,00 ₺' && (
                                                <div className="flex justify-between">
                                                    <span className="text-muted-foreground">İndirimler:</span>
                                        ...
                                        Object.entries(order.totals).map(([key, value]) => (
                                ...
                                <div className="border-t border-border pt-2.5 space-y-2">
                                    <div className="flex items-baseline justify-between">
                                        <span className="font-semibold text-xs text-foreground">Toplam Ödenen:</span>
                                        <span className="font-mono font-bold text-base text-foreground tabular-nums">
                                            {financials?.netSales ?? (Object.entries(order.totals).length > 0 ? (order.totals.net ?? order.totals.gross) : '—')}
                                        </span>
                                    </div>

                                    {financials?.netPayout && (
                                        <div className="rounded-lg border border-success/20 bg-success/5 p-2.5 space-y-0.5">
                                            ...
                                            <span className="block text-[10px] text-muted-foreground">
                                                Pazaryeri komisyonu düşüldükten sonra hesaba geçecek tutar.
                                            </span>
                                        </div>
                                    )}
                                    ...
                                    {financials?.estimatedProfit && (
                                        <div className="flex items-center justify-between text-xs pt-0.5">
                                            ...
                                            <span className="font-mono font-bold text-foreground tabular-nums">
                                                {financials.estimatedProfit} ({financials.marginPercent})
                                            </span>
                                        </div>
                                    )}
                                </div>
                        ...
                        <Card className="gap-0 py-0 overflow-hidden border-border bg-card shadow-xs">
                            ...
                            <CardContent className="p-4 space-y-2.5 text-xs">
                                <div>
                                    <span className="block text-[11px] text-muted-foreground font-medium">Alıcı Müşteri:</span>
                                    <span className="font-semibold text-foreground text-xs">
                                        {order.customer.name ?? '—'}
                                    </span>
                                </div>
                                ...
                                <div>
                                    <span className="block text-[11px] text-muted-foreground font-medium">Teslimat Bölgesi:</span>
                                    <div className="flex items-center gap-1.5 pt-0.5 text-foreground font-medium">
                                        <MapPin className="size-3.5 text-muted-foreground shrink-0" />
                                        <span>
                                            {[order.customer.district, order.customer.city].filter(Boolean).join(', ') || '—'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {order.customer.phone && (
                                    <div>
                                        <span className="block text-[11px] text-muted-foreground font-medium">İletişim Telefon:</span>
                                        <div className="flex items-center gap-1.5 pt-0.5 text-foreground font-mono">
                                ...
                                {order.customer.email && (
                                    <div>
                                        <span className="block text-[11px] text-muted-foreground font-medium">E-posta Adresi:</span>
                                        <span className="font-sans text-xs text-foreground truncate block pt-0.5">
                                            {order.customer.email}
                                        </span>
                                    </div>
                                )}
                        ...
                        <Card className="gap-0 py-0 overflow-hidden border-border bg-card shadow-xs">
                            ...
                            <CardContent className="p-4 space-y-2 text-xs">
                                <div className="flex items-center justify-between">
                                    <span className="text-muted-foreground">Kanal / Mağaza:</span>
                                    <span className="font-semibold text-foreground">
                                        {order.connection || 'Ana Mağaza'}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-muted-foreground">Pazaryeri Durumu:</span>
